            ...
            <div className="rounded-lg border border-border bg-surface-muted/50 p-2.5">
              {(() => {
                ...
                const checklist = [
                  ...
                  { label: "Đã cấu hình Kết nối",       done: agentConnectorStore.list(id ?? "new").length > 0, section: "instructions" },
...
function AiBuildSidebar({
  ...
  return (
    <aside className="w-[476px] border-r border-border flex flex-col shrink-0 h-full" style={{background:"hsl(var(--card))", minWidth:"476px"}}>
      {/* Header */}
...
/* ============ KNOWLEDGE ============ */
/** Shared row for a linked/owned knowledge source — used by both the sidebar "Tri thức" card
 * and the Knowledge screen's "Kho tri thức đã liên kết" section so the two read identically.
...
function KnowledgeSourceRow({ icon, name, chip, onOpen, onRemove, openLabel = "Mở nguồn tri thức", removeLabel = "Gỡ nguồn tri thức", disabled = false, disabledReason = "Nguồn tri thức đang được xử lý.", href, twoLine = false, hideOpen = false, toggle }: {
  ...
  twoLine?: boolean;
  /** Instructions sidebar "Tri thức" widget only (Round 4 Prompt G): the row isn't clickable to
   * open a detail/chunk view from here, and the "..." menu drops its open item entirely — only
  ...
  const row = href && !disabled && !hideOpen ? (
    <a href={href} target="_blank" rel="noopener noreferrer" className={rowClassName}>{rowInner}</a>
  ) : (
    <div
      role={disabled || hideOpen ? undefined : "button"}
      tabIndex={disabled || hideOpen ? undefined : 0}
      onClick={disabled || hideOpen ? undefined : onOpen}
      onKeyDown={disabled || hideOpen ? undefined : (e => { if (e.key === "Enter") onOpen(); })}
...
function NewConfigPanel({ agentId, model, onModelChange, onConnectionsChange }: { agentId: string; model: string; onModelChange: (id: string) => void; onConnectionsChange?: () => void }) {
  ...
  const sections = [
    ...
    {
      id: "knowledge", icon: NoteIcon, label: "Tri thức",
      onAdd: (pos: {top:number;left:number}) => knowledgeAddRef.current?.(pos),
      content: (
        <KnowledgeInner agentId={agentId} onRegisterAdd={(fn) => { knowledgeAddRef.current = fn; }} />
      ),
    },
    {
  ...
  return (
    <>
      {rows.length === 0 ? (
        <EmptyStateBox
          icon={BookOpen01Icon}
          description="Nguồn tri thức Agent tra cứu khi trả lời."
          addLabel="Thêm tri thức"
          onAdd={e => {
            const r = e.currentTarget.getBoundingClientRect();
            setMenuPos({ top: r.bottom + 4, left: r.right });
            setShowMenu(true);
        ...
        // No 4-item cap / "Xem tất cả" (Round 4 Prompt G) — the whole connected-knowledge list
        // renders here, scrolling internally when long instead of pushing the rest of the
        // Configuration sidebar down.
...
/** Namespaces a Custom Connector's id when it's attached via agentConnectorStore, so it never
 * collides with SUB_AGENT_CONNECTORS' plain ids ("gmail", "slack", ...). Shared by
 * ConnectorsInner (the Instructions-embedded "Kết nối" widget — the only place Custom
...
function AddCustomMcpModal({ onClose, onCreated }: { onClose: () => void; onCreated: (connector: CustomConnector) => void }) {
  ...
  const authOptions: { id: ConnectorAuthType | "oauth-auto" | "oauth-manual"; label: string; disabled: boolean }[] = [
    ...
    { id: "static_headers", label: "Static Headers",        disabled: false },
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2.5">
              {authOptions.map(opt => (
                <div key={opt.id}>
                  <label className={`flex items-center gap-2.5 ${opt.disabled ? "cursor-not-allowed" : "cursor-pointer"}`}>
                    <input
                      type="radio"
                      name="mcp-auth"
                      disabled={opt.disabled}
                      checked={authType === opt.id}
                      onChange={() => !opt.disabled && setAuthType(opt.id as ConnectorAuthType)}
                  ...
                  {authType === opt.id && opt.id === "static_headers" && (
                    <div className="mt-2 pl-6 space-y-2">
                        ...
                        <div key={i} className="flex items-center gap-1.5">
                          ...
                          <div className="relative flex-1">
                            <input
                              ...
                              placeholder="Bearer ..."
                              className="ds-input w-full h-9 pr-10"
                            />
                            <button type="button" onClick={() => toggleReveal(i)} className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-semibold text-muted-foreground hover:text-foreground">
                              {revealed.has(i) ? "Ẩn" : "Hiện"}
                            </button>
                          </div>
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        Key dùng chung tuỳ chọn cho cả workspace. Để trống để mỗi người tự kết nối riêng ở Workspace.
                      </p>
                    </div>
                  )}
                </div>
...
function ConnectorPickerModal({ connectors, customConnectors, added, onToggle, onClose, mode, onChangeMode, onCreatedCustom }: {
  ...
  const toggleAll = () => {
    ...
    else { allConnectors.forEach(c => { if (!added.includes(c.id)) onToggle(c.id); }); }
  };
