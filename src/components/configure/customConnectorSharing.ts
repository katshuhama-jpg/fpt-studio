/** Console-level ownership & sharing shape for Custom Connectors ("Custom MCP") — mirrors
 * Knowledge's own `Sharing`/`SharedPerson` model (src/components/knowledge/knowledgeBaseStore.ts)
 * field-for-field, so the two modules' sharing UI and permission behavior are identical, without
 * coupling Connectors to the Knowledge module's files. Same pattern as
 * src/components/configure/guardrailSharing.ts / skillSharing.ts. */
