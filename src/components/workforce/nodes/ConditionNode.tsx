...
export default function ConditionNode({ id, data, selected }: NodeProps<ConditionNodeData>) {
  const { onConfigure } = useWorkforceNodeActions();
  ...
  return (
    <div
      className={`min-w-[180px] max-w-[200px] rounded-full border bg-surface shadow-soft transition-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
        invalid ? "border-destructive ring-2 ring-destructive/25" : selected ? "border-primary ring-2 ring-primary/20" : "border-border"
      }`}
    >
      <NodeToolbarMenu visible={!!selected} onConfigure={() => onConfigure(id)} />
