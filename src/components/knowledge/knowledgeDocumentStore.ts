...
function seedKb(kbId: string) {
  ...
  if (kbId === "kb-1") {
    put({ id: "doc-1-f1", kbId, name: "Biểu phí & lãi suất", isFolder: true, folderId: null, status: "done", sizeBytes: 0, chunkCount: 0, version: 1, updatedAt: now - 3 * DAY, updatedBy: "Tran Nam" });
    put({ id: "doc-1-1", kbId, name: "Biểu lãi suất tiết kiệm 2026.pdf", isFolder: false, folderId: "doc-1-f1", status: "done", sizeBytes: 1_240_000, chunkCount: 42, version: 2, updatedAt: now - 2 * DAY, updatedBy: "Tran Nam" });
    put({ id: "doc-1-2", kbId, name: "Chính sách khiếu nại.docx", isFolder: false, folderId: null, status: "done", sizeBytes: 340_000, chunkCount: 18, version: 1, updatedAt: now - 5 * DAY, updatedBy: "Tran Nam" });
    put({ id: "doc-1-3", kbId, name: "Quy trình mở thẻ tín dụng.pdf", isFolder: false, folderId: null, status: "processing", sizeBytes: 2_100_000, chunkCount: 0, version: 1, updatedAt: now - 5 * 60_000, updatedBy: "Tran Nam" });
    put({ id: "doc-1-4", kbId, name: "Sổ tay sản phẩm vay.pptx", isFolder: false, folderId: null, status: "failed", statusReason: "Không đọc được nội dung tệp. Thử tải lại hoặc dùng bản PDF.", sizeBytes: 8_400_000, chunkCount: 0, version: 1, updatedAt: now - DAY, updatedBy: "Tran Nam" });
    put({ id: "doc-1-5", kbId, name: "Câu hỏi khiếu nại thường gặp.csv", isFolder: false, folderId: null, status: "pending", sizeBytes: 90_000, chunkCount: 0, version: 1, updatedAt: now - 60_000, updatedBy: "Tran Nam" });
  }

  if (kbId === "kb-3") {
    put({ id: "doc-3-1", kbId, name: "Quy trình xử lý sự cố hạ tầng.pdf", isFolder: false, folderId: null, status: "done", sizeBytes: 980_000, chunkCount: 55, version: 3, updatedAt: now - 4 * DAY, updatedBy: "Linh Phan" });
    put({ id: "doc-3-2", kbId, name: "Mẫu email thông báo bảo trì.docx", isFolder: false, folderId: null, status: "done", sizeBytes: 60_000, chunkCount: 6, version: 1, updatedAt: now - 10 * DAY, updatedBy: "Tran Nam" });
    put({ id: "doc-3-3", kbId, name: "Runbook triển khai phiên bản mới.md", isFolder: false, folderId: null, status: "done", sizeBytes: 120_000, chunkCount: 31, version: 5, updatedAt: now - DAY, updatedBy: "Duy Nguyen" });
  }
...
export const knowledgeDocumentStore = {
  ...
  createFolder(kbId: string, name: string, folderId: string | null = null): KnowledgeDocument {
    ...
    const rec: KnowledgeDocument = {
      ...
      sizeBytes: 0, chunkCount: 0, version: 1, updatedAt: Date.now(), updatedBy: "Tran Nam",
    };
    store.set(id, rec);
    persist();
    return rec;
